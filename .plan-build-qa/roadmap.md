...
| spec-017-catalogo-de-sensores | em andamento | 2 | 2026-05-25 | Package 1 fechado com Score 1 (`evaluations/package-1.md`). `templates/sensor-catalog.json` com 6 entradas (sonar-dotnet, sonar-js, dotnet-build, dotnet-test, eslint, playwright-e2e). `pbq sensor catalog` lista entradas marcando as cadastradas. `pbq sensor add --from-catalog <id>` adiciona com `source:"catalog"`. Convite nao-interativo no fim de `init`/`update`. Testes em `pbq-init-smoke.mjs`. | Implementar package 2: campo `phase` (before/after) no schema de sensores |
