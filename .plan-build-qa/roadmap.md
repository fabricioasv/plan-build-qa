...
| spec-011-test-as-independent-gate | concluido | 1 | 2026-05-25 | Package 1 fechado com Score 1 (`evaluations/package-1.md`, `npm-run-test` exit 0 + grep dos AC 1-6). Skill `test` reescrita em dois modos (contract-check/acceptance-check) como subagente de contexto fresco; `implement` delega verificacao (nao roda mais sensores) e `spec` invoca contract-check; `constitution/testing.md` codifica o pipeline de 5 etapas e a verificacao independente; quadro de 5 etapas adicionado em Estado Atual das duas copias do template de `progress.md`. Bypass = `skip test`. Pendencia para spec futura: propagar skills para `templates/adapters/skills/**` | - |
