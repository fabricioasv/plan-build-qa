...
| spec-014-melhora-deteccao-sensores | em andamento | 1 | 2026-05-23 | Reportado pelo usuario em 2026-05-23 ao tentar usar `/sensor` em um projeto real: `pbq init` nao detectou um script de teste existente nem um `sonar.bat` configurado, e a experiencia geral de cadastro via `pbq sensor add` exige muita digitacao manual (precisa saber nome, tier, comando e razao). A skill `/sensor` no estado atual nao orienta sobre o que cadastrar | Implementar `contracts/package-1.md` ampliando a deteccao do `inspectProject`/`detectCommands` para incluir scripts soltos (`.bat`, `.cmd`, `.sh`, `.ps1` em raiz ou `scripts/`), `Makefile` e padroes comuns como `sonar*`, depois `contracts/package-2.md` adicionando `pbq sensor suggest` que escaneia o repo e imprime comandos `pbq sensor add` prontos, depois `contracts/package-3.md` refinando a skill `/sensor` (Claude e Codex) com exemplos e fluxo recomendado usando o novo `suggest` |
